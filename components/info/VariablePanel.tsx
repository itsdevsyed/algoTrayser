// components/info/VariablePanel.tsx
...
export const VariablePanel: React.FC<VariablePanelProps> = ({
  ...
  const defaultVariables = [
    ...
    { key: 'x', label: 'Current Value', value: step?.value !== undefined ? step.value : '-' },
    { key: 'complement', label: 'Complement', value: step?.complement !== undefined ? step.complement : '-' },
  ];

  const variables = customVariables ? Object.entries(customVariables) : defaultVariables.map(v => [v.key, v]);
    ...
    <div className={`bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-xl transition-colors ${className}`}>
      ...
      <div className="space-y-3">
        {variables.map(([key, config]) => {
          const isCustom = customVariables && customVariables[key];
          const label = isCustom ? config.label : key;
          const value = isCustom ? config.value : step?.[key as keyof Step];
          const color = isCustom ? config.color || 'text-sky-500' : 'text-sky-500';
